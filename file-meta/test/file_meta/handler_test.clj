...
(deftest test-app
  ...
  (testing "find a single record's catalog id only"
    (let [response (app
                    (mock/request
                     :get
                     "/series/test-series/1/1?catalog_id_only=true"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
  ...
  (testing "find all records catalog_ids"
    (let [response (app
                    (mock/request
                     :get
                     "/series/test-series?catalog_id_only=true"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"status" "ok",
              "catalog_ids" ["TESTS0101001",
                            ...
                            (mock/json-body
                             {
                              ...
                              "summary" (str "Wonderful story of cats "
                                             "being cats and everyone "
                                             "loving them. Hooray!")
                              "imdbid" "tt6565"
  ...
  (testing "can't update nonexistant stub record"
    (let [response (app (-> (mock/request
                             :put "/series/does-not-exist-series/1/1")
                            (mock/json-body
                             {
                              "episode_name" "The Cat Returns"
                              "summary" (str "Wonderful story of cats "
          ...
          (app (-> (mock/request :put "/series/test-series")
                   (mock/json-body
                    {"series" {"imdbid" "tt222222"
                     ...
                     "records" [{
                                 "episode_name" "The Cat Returns 2"
                                 "summary" (str "Wonderful story of "
                                                "cats being cats and "
  ...
  (testing "find specific fields of a second record"
    (let [response (app (mock/request
                         :get "/series/test-series/1/2?fields=summary"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"status" "ok",
              "catalog_ids"["TESTS0101002"],
              "records"
              [{"summary" (str "Wonderful story of cats being cats "
                               "and everyone loving them. Hooray! For "
                               "the second time")}]}))))
  (testing "verify catalog ids are created correctly for subsequent records"
    (let [response (app (mock/request :post "/series/test-serials/1/1"))
      ...
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"status" "ok","catalog_ids" ["TESTS0201001"]}))
      (is (= (:status response2) 200))
      (is (= (parse-string (:body response2))
             {"status" "ok","catalog_ids" ["TESTS0301001"]}))))
  (testing "get list of available series"
    (let [response (app (mock/request :get "/series"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"status" "ok",
              "results" ["test-serials",
                         "test-serials2",
                         "test-series"]}))))
    ...
    (let [response (app (mock/request :get "/catalog-id/TESTS0101001"))]
      ...
      (is (= (parse-string (:body response))
             {"status" "ok",
              ...
              "records" [
                         {"episode_name" "The Cat Returns",
                          "summary" (str "Wonderful story of cats being "
                                         "cats and everyone loving them. "
                                         "Hooray!"),
                          "episode" 1,
                          "season" 1,
                          "series" "test-series",
  ...
  (testing "find specific fields of a second record by catalog_id"
    (let [response (app (mock/request
                         :get "/catalog-id/TESTS0101003?fields=summary"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"status" "ok",
              "catalog_ids"["TESTS0101003"],
              "records"
              [{"summary" (str "Wonderful story of cats being cats "
                               "and everyone loving them. Hooray! "
                               "Got tired of this")}]}))))
...
(deftest post-series
  ...
  (testing "find previous series by catalog-id"
    (let [response (app (mock/request :get "/catalog-id/NEWT00132015"))]
      (is (= (:status response) 200))
      (is (= (parse-string (:body response))
             {"records" [{
                          "series" "new-t"

(ns remote-call.validate-test
  (:require [remote-call.validate :refer :all]
...
(deftest test-validation
  ...
  (testing "full playlist passing"
    (let [testing {:name "something"
                    :playlists [
                              {:type "complex"
                              :playlists [{:type "playlist" :name "a" :length 13}
                                          {:type "playlist" :name "b" :length 14}]}
                              {:type "merge"
                              :playlists [{:type "playlist" :name "c" :length 12}
  ...
  (testing "full playlist failing"
    (let [testing {:name "something"
          ...
          playlist-map {"a" 13 "b" 14 "c" 12 "d" 16 "e" 18 "f" 20}
          expected {:status :invalid :messages ["Failed Validation: a" "Failed Validation: c", "Failed Validation: f"]}
          actual (validate-schedule playlist-map "something" testing)]

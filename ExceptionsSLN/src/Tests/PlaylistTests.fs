...
let ``Invalid playlist?``() =
    ...
    let actual = catalogIds input verifier
    Assert.Equal(Failures(Message "Catalog ID does not exist", Errors ["TEST0101001";"TEST0101002";"TEST0101003"]),
        actual)
...
let ``Partial failure?``() =
    ...
    let actual = catalogIds input verifier
    Assert.Equal(Failures(Message "Catalog ID does not exist", Errors ["TEST0101002"]),
        actual)